...
def login():
	...
	try:
		head = {"User-Agent": "Mozilla/5.0 (Linux; Android 8.1.0; CPH1909) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Mobile Safari/537.36"}
...
def menu():
	...
	os.system('clear')
	banner()
	prints(panel(f"""[green][[green]1[green]] Crack Publik[green]] \n[[green]2[green]] Crack Massal[hijau][green]] \n[[green]3[green]] logout [red][[red]hapus cookies[red]] """,width=43,title=f"[[green] SILAHKAN PILIH MENU[/]]",style=f"yellow"))
	print(f"{mer}≥-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-≤{puti}")
	CYXIEON_GANTENG = input(f'{mer}\==[{hijo} Input menu : ')
...
def dump_massal():
	...
	for user in uid:
	    try:
	       head = (
	       {"user-agent": "Mozilla/5.0 (Linux; Android 8.1.0; CPH1909) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Mobile Safari/537.36"
	       })
	       if len(id) == 0:
	           params = (
	           {
...
def passwordlist():
	...
	print(f"{mer}≥-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-≤{puti}")
	print(f'{kun}\==[{puti} OK {kun}: %s'%(ok))
	print(f'{kun}\==[{puti} CP {mer}: %s'%(cp))
	print(f"{mer}≥-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-_-≤{puti}")
...
def crackasyinc(idf,pwx):
  ...
  for pw in pwx:
    try:
      ...
      ua = rc(ugen)
      ua2 = rc(["Mozilla/5.0 (Linux; Android 8.1.0; CPH1909) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.108 Mobile Safari/537.36","Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36 Edg/91.0.864.59"])
